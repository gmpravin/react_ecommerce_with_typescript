...
const Register = () => {
  ...
  const validateData = () => {
    ...
    if (
      ...
      setErr({ comerr: 'Fields must be filled' });
      ...
      console.log('name');
      setErr({ nameerr: 'Name required' });
    } else if (mobileno.length === 0) {
      console.log('no');
      setErr({ mobilenoerr: 'Mobileno required' });
    } else {
      handleLogin();
    }
  };
  ...
  const handleLogin = async () => {
    ...
    setLoading(true);
    await fetch('http://localhost:8000/api/signup', requestOptions)
  ...
  return (
    <div className={style.registerpage}>
      <div className={style.container}>
        <div>
          <img src={RegisterImg} className={style.img} />
        </div>
        <div className={style.form}>
          <div className={style.title}>Register a account</div>
          {err ? (
            <Fragment>
          ...
          {err.emailerr ? (
            <div className={style.alert}>{err.emailerr}</div>
          ) : null}
          ...
          {err.passworderr ? (
            <div className={style.alert}>{err.passworderr}</div>
          ) : null}
          ...
          {err.nameerr ? (
            <div className={style.alert}>{err.nameerr}</div>
          ) : null}
          ...
          {err.mobilenoerr ? (
            <div className={style.alert}>{err.mobilenoerr}</div>
          ) : null}
          <button
            type="button"
